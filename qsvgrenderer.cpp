...
// added by yuanls
QRectF QSvgRenderer::getNodeTransformedBounds(const QString& id)
{
	 Q_D(const QSvgRenderer);
	 QRectF boundRect;
	 if (d->render)
	 {
		 boundRect = d->render->scopeNode(id)->transformedBounds();
	 }
	 return boundRect;
}

void QSvgRenderer::getViewBoxOnElement(const QString& id,qreal& xp,qreal &yp)
{
	Q_D(const QSvgRenderer);
	if (d->render)
	{
		QSvgNode* node = d->render->scopeNode(id);
		if (node != NULL)
		{
			xp = node->m_xViewBox;
			yp = node->m_yViewBox;
