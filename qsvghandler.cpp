	...
	QSvgHandler *handler)
{
	...
	if (viewBoxValues.count() == 4) {
		...
		QSvgHandler::LengthType lt;
		x = parseLength(xStr, lt, handler);
		y = parseLength(yStr, lt, handler);
		qreal w = parseLength(widthStr, lt, handler);
		qreal h = parseLength(heightStr, lt, handler);
	}

	//Q_UNUSED(attributes);
	QSvgSymbol *node = new QSvgSymbol(parent);
	node->m_xViewBox = x;
	node->m_yViewBox = y;
...
void QSvgHandler::parse()
{
    ...
    while (!xml->atEnd() && !done) {
        ...
        case QXmlStreamReader::Characters:
			test = xml->text().toString();
            characters(xml->text());
            break;
        case QXmlStreamReader::ProcessingInstruction:
			test = xml->processingInstructionTarget().toString();
